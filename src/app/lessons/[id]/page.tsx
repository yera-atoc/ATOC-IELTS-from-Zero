...
import { getLessonsByBand } from '@/data'; // Импортируем только то, что нужно
...
function getBandSlots(band: CEFRBand, available: any[]) {
  // Приводим ID из базы к нижнему регистру для надежного сравнения
  ...
  return Array.from({ length: 20 }, (_, i) => {
    ...
    return {
      id: id, // будет например 'b1'
      title: lesson?.title ?? `Урок ${band.toUpperCase()}${i + 1}`,
      available: !!lesson,
    };
  });
}
...
export default function LessonsPage() {
  ...
  // Получаем список уроков для текущего уровня
